...
// Check the exam
// https://www.codewars.com/kata/5a3dd29055519e23ec000074/train/
...
console.log(decode('761328qockcouoqmoayqwmkkic'), 'Impossible to decode')
...
function decode(text) {
  const number = text.match(/\d+/g)[0]
  
  for (let index = 2; index < 27; index++) {
    if (26 % index === 0 && +number % index === 0) {
      return 'Impossible to decode'
    }
  }
  
  const word = text.match(/[a-z]+/g)[0]
  let decodedLetters = '';
  ...
  for (const letter of word) {
    ...
    for (const alpha of letters) {
      if (letters.indexOf(alpha) * number % 26 === indexOfLetter) {
        decodedLetters += alpha
        break
      }
    }
  }
...
// https://www.codewars.com/kata/56fc55cd1f5a93d68a001d4e/train/
...
function stairsIn20(stairs) {
  let stairsInOneYear = 0;

  for (const stair of stairs) {
    stairsInOneYear += stair.reduce((total, current) => total + current);
  }
...
function stairsIn20(stairs) {
  return stairs.flat().reduce((total, current) => total + current) * 20
}
